 ...
 * @since 	Mar 9, 2016
 */
public class NamedEntityChunker<N extends NLPNode> extends Chunker<N>{
	private Set<String> target_tags;
	
	public NamedEntityChunker(Set<String> target_tags){
		...
		for(start = 1; start < nodes.length; start++){
			...
			case 'B':
				...
				if(target_tags.contains(tag)){
					...
					chunks.add(new Chunk<>(tag, (N[])Arrays.copyOfRange(nodes, start, end+1)));
					start = end;					
				}
				break;
			case 'U':
				tag = node.getNamedEntityTag().substring(2);
				if(target_tags.contains(tag))
					chunks.add(new Chunk<>(tag, (N[])Arrays.copyOfRange(nodes, start, start+1)));
				break;
			}
		}
		return chunks;
